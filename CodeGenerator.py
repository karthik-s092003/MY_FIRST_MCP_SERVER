...
"""
    ...
    await file_agent.run(
        task=f"Generate and write all the files from the list of files (Files) generated by previous agent make sure you don't miss out on any file from the list. Use the project description: {user_prompt}"
    )
